...
namespace :poll do
  ...
  task servers: :environment do
    ...
    Server.all.each do |server|
      ...
      server.load = (video_streams * weight_videos + server_users * weight_users + meetings.length * weight_meetings) * (server.load_multiplier.nil? ? 1.0 : server.load_multiplier.to_d)
      server.online = true
